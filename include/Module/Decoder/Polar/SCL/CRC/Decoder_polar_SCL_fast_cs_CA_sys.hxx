...
Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar>
::Decoder_polar_SCL_fast_cs_CA_sys(const int& K, const int& N, const int& L, const std::vector<bool>& frozen_bits,
                                const CRC<B>& crc)
: Decoder_polar_SCL_fast_cs_sys<B,R,API_polar>(K, N, L, frozen_bits),
  fast_store(false), crc(crc.clone()), U_test(K)
{
	const std::string name = "Decoder_polar_SCL_fast_cs_CA_sys";
	this->set_name(name);
...
: Decoder_polar_SCL_fast_cs_sys<B,R,API_polar>(K, N, L, frozen_bits, polar_patterns, idx_r0, idx_r1),
  fast_store(false), crc(crc.clone()), U_test(K)
{
	const std::string name = "Decoder_polar_SCL_fast_cs_CA_sys";
	this->set_name(name);
...
template <typename B, typename R, class API_polar>
Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar>* Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar>
::clone() const
{
	auto m = new Decoder_polar_SCL_fast_cs_CA_sys(*this);
	m->deep_copy(*this);
	return m;
}

template <typename B, typename R, class API_polar>
void Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar>
::deep_copy(const Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar> &m)
{
	Decoder_polar_SCL_fast_cs_sys<B,R,API_polar>::deep_copy(m);
	if (m.crc != nullptr) this->crc.reset(m.crc->clone());
}

template <typename B, typename R, class API_polar>
bool Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar>
...
template <typename B, typename R, class API_polar>
int Decoder_polar_SCL_fast_cs_CA_sys<B,R,API_polar>
...
::init_buffers()
{
	Decoder_polar_SCL_fast_cs_sys<B,R,API_polar>::init_buffers();
	fast_store = false;
}
...
::_store(B *V_K) const
{
	if (fast_store)
		...
		Decoder_polar_SCL_fast_cs_sys<B,R,API_polar>::_store(V_K);
}
}
}
