#include <sstream>

#include "Tools/Exception/exception.hpp"
#include "Tools/Code/Polar/Patterns/Pattern_polar_r1.hpp"
#include "Tools/Code/Polar/Patterns/Pattern_polar_t4.hpp"

namespace aff3ct
{
namespace tools
{
Pattern_polar_t4
::Pattern_polar_t4(const int &N, const Binary_node<Pattern_polar_i>* node, const int min_level, const int max_level)
: Pattern_polar_i(N, node, min_level, max_level)
{
	if (min_level < 1)
	{
		std::stringstream message;
		message << "'min_level' has to be equal or greater than 1 ('min_level' = " << min_level << ").";
		throw invalid_argument(__FILE__, __LINE__, __func__, message.str());
	}
}

Pattern_polar_t4
::Pattern_polar_t4(const int min_level, const int max_level)
: Pattern_polar_i(min_level, max_level)
{
	if (min_level < 1)
	{
		std::stringstream message;
		message << "'min_level' has to be equal or greater than 1 ('min_level' = " << min_level << ").";
		throw invalid_argument(__FILE__, __LINE__, __func__, message.str());
	}
}

Pattern_polar_t4* Pattern_polar_t4
::alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const
{
	if (node == nullptr)
		return new Pattern_polar_t4(min_level, max_level);
	else
		return new Pattern_polar_t4(N, node, min_level, max_level);
}

polar_node_t Pattern_polar_t4
::type()       const
{
	// todo 
	return polar_node_t::T4;
}

std::string  Pattern_polar_t4
::name()       const
{
	return "T4";
}

std::string  Pattern_polar_t4
::short_name() const
{
	return "4";
}

std::string  Pattern_polar_t4
::fill_color() const
{
	return "#2F3FFF";
}

std::string  Pattern_polar_t4
::font_color() const
{
	return "#FFFFFF";
}

std::string Pattern_polar_t4
::f() const
{
	return "";
}

std::string Pattern_polar_t4
::g() const
{
	return "";
}

std::string Pattern_polar_t4
::h() const
{
	return "t4";
}

bool Pattern_polar_t4
::recursive_check(const int reverse_graph_depth, const Binary_node<Pattern_polar_i>* node_curr)
{
	if (!node_curr->is_leaf())
	{
		const auto pattern_left  = node_curr->get_left ()->get_contents();
		const auto pattern_right = node_curr->get_right()->get_contents();

		if (pattern_right->type() == polar_node_t::RATE_1 ||
		    Pattern_polar_r1::recursive_check(reverse_graph_depth -1, node_curr->get_right()))
		{
			if (pattern_left->type() == polar_node_t::T4)
				return true;
			else
				return Pattern_polar_t4::recursive_check(reverse_graph_depth -1, node_curr->get_left());
		}
		else if (pattern_left->type() == polar_node_t::RATE_0 && pattern_right->type() == polar_node_t::REP && reverse_graph_depth == 2)
		{
			return true;
		}
		else 
		{
			return false;
		}
	}
	else
	{
		return false;
	}
}

int Pattern_polar_t4
::_match(const int &reverse_graph_depth, const Binary_node<Pattern_polar_i>* node_curr) const
{
	// todo 49 ?
	return Pattern_polar_t4::recursive_check(reverse_graph_depth, node_curr) ? 47 : 0;
}

bool Pattern_polar_t4
::is_terminal() const
{
	return true;
}
}
}
