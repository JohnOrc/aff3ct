...
{
Pattern_polar_t2
::Pattern_polar_t2(const int &N, const Binary_node<Pattern_polar_i>* node, const int min_level, const int max_level)
...
Pattern_polar_t2
::Pattern_polar_t2(const int min_level, const int max_level)
...
Pattern_polar_t2* Pattern_polar_t2
::alloc(const int &N, const Binary_node<Pattern_polar_i>* node) const
{
	if (node == nullptr)
		return new Pattern_polar_t2(min_level, max_level);
	else
		return new Pattern_polar_t2(N, node, min_level, max_level);
}

polar_node_t Pattern_polar_t2
::type()       const
{
	// todo 
	return polar_node_t::T2;
}

std::string  Pattern_polar_t2
::name()       const
{
	return "t2";
}
...
::recursive_check(const int reverse_graph_depth, const Binary_node<Pattern_polar_i>* node_curr)
{
	if (!node_curr->is_leaf())
	{
		...
		{
			if (pattern_right->type() == polar_node_t::T2)
				return true;
			else 
				return Pattern_polar_t2::recursive_check(reverse_graph_depth -1, node_curr->get_right());
		}
		else if (reverse_graph_depth == 2 && pattern_left->type() == polar_node_t::REP && pattern_right->type() == polar_node_t::RATE_1)
			return true;
		else
		{
			return false;
...
int Pattern_polar_t2
::_match(const int &reverse_graph_depth, const Binary_node<Pattern_polar_i>* node_curr) const
{
	return Pattern_polar_t2::recursive_check(reverse_graph_depth, node_curr) ? 81 : 0;
}
